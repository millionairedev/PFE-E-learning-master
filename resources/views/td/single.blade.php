...
@section('category')
 ...
 <nav class="nav-sidebar">
                <ul class="nav">
                    ...
                    <li><a href="{{route('td.index')}}" class="list-group-item disabled">
  <span class="badge "></span>--Liste matieres--</a></li>
                    @foreach($matieres as $key => $value)
    <li><a href="{{route('td.index',['matiere'=>$key])}}" class="list-group-item">
...
<br>
@if($taf->fichier != null)
<iframe  width="100%" height="400px" src="{{asset("storage/$td->fichier")}}"> 
</iframe>
@endif
...
<div class="actions">
	<a href="{{route('td.edit',$td->id)}}" class="btn btn-warning btn-xs" method="POST">Modifier</a>

	<form action="{{route('td.destroy',$td->id)}}" method="POST" class="inline-it">
	{{csrf_field()}}
	{{method_field('DELETE')}}
